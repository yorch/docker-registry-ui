...
- **`DOCKER_REGISTRY_UI_TITLE` is now `APP_NAME`** and the old name is no longer read. If you set it upstream, rename it; the header falls back to `Registry Explorer` otherwise, which is visible immediately and harmless until you get to it.
...
- `APP_NAME`: A custom name for **the application**, shown in the header bar. Distinct from `REGISTRY_TITLE`, which names the registry you are browsing. (default: `Registry Explorer`). Since 3.2.0, replacing `DOCKER_REGISTRY_UI_TITLE`.
